"""
math_tiles_oop_v2.py
OOP refactor of mypygame_optimized.py (single-file).
Preserves gameplay, visuals, fonts, and layout exactly as in the original.
"""

import pygame
import random
import json
from pathlib import Path

# ---------------- Constants ---------------- #
SCREEN_W, SCREEN_H = 800, 600
FPS = 60
SAVE_PATH = Path(__file__).parent / "savegame.json"

# Colors
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
TEAL = (30, 200, 180)
DARK = (20, 20, 20)
GREEN = (60, 200, 60)
RED = (200, 50, 50)
YELLOW = (230, 200, 40)
GRAY = (160, 160, 160)
PANEL_BG = (18, 18, 18)

# Gameplay tuning
BASE_FALL_SPEED = 60
SPEED_PER_SCORE = 3
SPAWN_INTERVAL_BASE = 2.0
SPAWN_INTERVAL_MIN = 0.5
SPAWN_ACCEL_PER_SCORE = 0.03

# Item costs
COST_SKIP = 8
COST_SHIELD = 15

# ---------------- Save Manager (keeps same names / behavior) ---------------- #
def load_save():
    if not SAVE_PATH.exists():
        return {"high_score": 0, "coins": 0, "items": {"skip": 0, "shield": 0}}
    try:
        with open(SAVE_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
        data.setdefault("high_score", 0)
        data.setdefault("coins", 0)
        data.setdefault("items", {"skip": 0, "shield": 0})
        return data
    except Exception:
        return {"high_score": 0, "coins": 0, "items": {"skip": 0, "shield": 0}}

def save_game(data):
    try:
        with open(SAVE_PATH, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2)
    except Exception as e:
        print("Save failed:", e)

# ---------------- Utilities ---------------- #
def generate_problem():
    op = random.choice(['+', '-', '*', '/'])
    a = random.randint(1, 12)
    b = random.randint(1, 12)
    if op == '/':
        b = random.randint(1, 12)
        a = b * random.randint(1, 6)
        answer = a // b
        display = f"{a} ÷ {b}"
    elif op == '+':
        answer = a + b
        display = f"{a} + {b}"
    elif op == '-':
        answer = a - b
        display = f"{a} - {b}"
    else:
        answer = a * b
        display = f"{a} × {b}"
    return display, answer

def make_choices(correct):
    choices = {correct}
    while len(choices) < 3:
        delta = random.randint(-10, 10)
        candidate = correct + delta
        if candidate != correct and -100 <= candidate <= 100:
            choices.add(candidate)
    choices = list(choices)
    random.shuffle(choices)
    return choices

# ---------------- ProblemTile ---------------- #
class ProblemTile:
    WIDTH = 300
    HEIGHT = 70

    def __init__(self, x, y, problem_str, answer, fall_speed):
        self.x = x
        self.y = float(y)
        self.problem = problem_str
        self.answer = answer
        self.width = ProblemTile.WIDTH
        self.height = ProblemTile.HEIGHT
        self.fall_speed = fall_speed
        self.rect = pygame.Rect(self.x, int(self.y), self.width, self.height)
        self.choices = make_choices(answer)

    def update(self, dt):
        self.y += self.fall_speed * dt
        self.rect.y = int(self.y)

    def draw(self, surf, font):
        pygame.draw.rect(surf, TEAL, self.rect, border_radius=8)
        txt = font.render(self.problem, True, DARK)
        surf.blit(txt, (self.rect.x + 12, self.rect.y + (self.height - txt.get_height()) // 2))
        pygame.draw.rect(surf, BLACK, self.rect, 2, border_radius=8)

# ---------------- UI Functions (kept visually identical) ---------------- #
class UIManager:
    def __init__(self, screen):
        self.screen = screen
        # keep fonts identical
        self.font_title = pygame.font.SysFont("Comic Sans MS", 50, bold=True)
        self.font_menu_btn = pygame.font.SysFont("Comic Sans MS", 36, bold=True)
        self.font_big = pygame.font.SysFont("Comic Sans MS", 48)
        self.font_mid = pygame.font.SysFont("Comic Sans MS", 36)
        self.font_body = pygame.font.SysFont("Comic Sans MS", 22)
        self.font_small = pygame.font.SysFont("Comic Sans MS", 20)

    def draw_right_panel(self, answer_buttons):
        panel_w = 200
        panel_rect = pygame.Rect(SCREEN_W - panel_w - 20, 60, panel_w, SCREEN_H - 140)
        pygame.draw.rect(self.screen, PANEL_BG, panel_rect, border_radius=12)
        pygame.draw.rect(self.screen, (40, 40, 40), panel_rect, 3, border_radius=12)
        for i, (rect, val) in enumerate(answer_buttons):
        # check for flash color
            color = GREEN
            if hasattr(self, "button_flash") and self.button_flash[i]:
                color = self.button_flash[i]
            pygame.draw.rect(self.screen, color, rect, border_radius=10)
            txt = self.font_big.render(str(val), True, DARK)
            self.screen.blit(txt, (rect.x + (rect.width - txt.get_width()) // 2,
                                rect.y + (rect.height - txt.get_height()) // 2))

    def draw_hud(self, score, coins, lives, high_score):
        self.screen.blit(self.font_small.render(f"Score: {score}", True, WHITE), (20, 10))
        self.screen.blit(self.font_small.render(f"Coins: {coins}", True, YELLOW), (20, 40))
        self.screen.blit(self.font_small.render(f"Lives: {lives}", True, RED), (180, 10))
        self.screen.blit(self.font_small.render(f"High Score: {high_score}", True, GRAY), (180, 40))

    def draw_shop(self, player_items, player_coins):
        w, h = 800, 600
        rect = pygame.Rect((SCREEN_W - w) // 2, (SCREEN_H - h) // 2, w, h)
        overlay = pygame.Surface((SCREEN_W, SCREEN_H))
        overlay.fill((0,0,0))
        self.screen.blit(overlay, (0,0))
        pygame.draw.rect(self.screen, (30, 30, 30), rect, border_radius=12)
        pygame.draw.rect(self.screen, (80, 80, 80), rect, 2, border_radius=12)
        y = rect.y + 48
        self.screen.blit(self.font_small.render(f"SHOP - Buy Items           Coin: {player_coins}", True, WHITE), (rect.x + 18, rect.y + 12))
        self.screen.blit(self.font_small.render(f"Skip (cost {COST_SKIP}) - You have: {player_items['skip']}", True, WHITE), (rect.x + 20, y))
        y += 46
        self.screen.blit(self.font_small.render(f"Shield (cost {COST_SHIELD}) - You have: {player_items['shield']}", True, WHITE), (rect.x + 20, y))
        y += 60
        self.screen.blit(self.font_small.render("Click item text to buy. Press S to close shop.", True, GRAY), (rect.x + 20, y))
        return rect

    def draw_menu(self):
        screen = self.screen
        screen.fill((100, 100, 100))
        title = self.font_title.render("Math Tiles", True, WHITE)
        screen.blit(title, (SCREEN_W//2 - title.get_width()//2, 110))

        buttons = {
            "Play": pygame.Rect(SCREEN_W//2 - 100, 300, 200, 60),
            "Tutorial": pygame.Rect(SCREEN_W//2 - 100, 390, 200, 60),
            "Quit": pygame.Rect(SCREEN_W//2 - 100, 480, 200, 60)
        }
        pygame.draw.rect(screen, GREEN, buttons["Play"], border_radius=12)
        pygame.draw.rect(screen, YELLOW, buttons["Tutorial"], border_radius=12)
        pygame.draw.rect(screen, RED, buttons["Quit"], border_radius=12)

        for label, rect in buttons.items():
            txt = self.font_menu_btn.render(label, True, WHITE)
            screen.blit(txt, (rect.centerx - txt.get_width()//2, rect.centery - txt.get_height()//2))

        return buttons

    def draw_tutorial(self):
        screen = self.screen
        screen.fill(BLACK)
        title = self.font_title.render("How to Play Math Tiles", True, WHITE)
        screen.blit(title, (SCREEN_W//2 - title.get_width()//2, 30))

        lines = [
            "Solve falling math problems before they reach the bottom!",
            "Click the correct answer on the right panel.",
            "You start with 3 lives — lose one for each wrong or missed tile.",
            "Earn 1 coin for each correct answer.",
            "Press S to open the shop:",
            "  - Buy 'Skip' to remove the current problem.",
            "  - Buy 'Shield' to block one mistake.",
            "Controls:",
            "  S - Open/close shop",
            "  K - Use Skip item",
            "  ESC - Back to menu",
            "Click 'Back' below to return to menu."
        ]
        y = 120
        for line in lines:
            txt = self.font_body.render(line, True, WHITE)
            screen.blit(txt, (60, y))
            y += 34

        back_btn = pygame.Rect(SCREEN_W//2 - 60, SCREEN_H - 60, 120, 45)
        pygame.draw.rect(screen, TEAL, back_btn, border_radius=10)
        txt = self.font_body.render("Back", True, BLACK)
        screen.blit(txt, (back_btn.centerx - txt.get_width()//2, back_btn.centery - txt.get_height()//2))
        return back_btn

# ---------------- Game State ---------------- #
class GameState:
    def __init__(self, save_data):
        # persistent
        self.save_data = save_data
        self.high_score = save_data.get("high_score", 0)
        self.coins = save_data.get("coins", 0)
        self.items = save_data.get("items", {"skip": 0, "shield": 0})
        # per-run
        self.tiles = []
        self.score = 0
        self.lives = 3
        self.spawn_timer = 0.0
        self.spawn_interval = SPAWN_INTERVAL_BASE
        self.game_over = False
        self.shop_open = False

    def spawn_tile(self):
        fall_speed = BASE_FALL_SPEED + self.score * SPEED_PER_SCORE
        x = 40
        y = 75  # preserved from mypygame_optimized.py
        pstr, ans = generate_problem()
        self.tiles.append(ProblemTile(x, y, pstr, ans, fall_speed))

    def reset_run(self):
        self.tiles = []
        self.score = 0
        self.lives = 3
        self.spawn_timer = 0.0
        self.spawn_interval = SPAWN_INTERVAL_BASE
        self.game_over = False
        self.shop_open = False

    def update_save_values(self):
        self.save_data["high_score"] = max(self.high_score, self.save_data.get("high_score", 0))
        self.save_data["coins"] = self.coins
        self.save_data["items"] = self.items

# ---------------- Controller: GameApp ---------------- #
class GameApp:
    def __init__(self):
        pygame.init()
        self.screen = pygame.display.set_mode((SCREEN_W, SCREEN_H))
        pygame.display.set_caption("Math Tiles — Math Game")
        self.clock = pygame.time.Clock()
        self.ui = UIManager(self.screen)

        self.save_data = load_save()
        self.high_score = self.save_data.get("high_score", 0)

        self.state = "menu"  # "menu", "tutorial", "game"
        self.gs = GameState(self.save_data)

        # answer buttons layout
        button_w, button_h = 160, 80
        right_x = SCREEN_W - button_w - 40
        btn_y_positions = [140, 260, 380]
        self.answer_buttons = [(pygame.Rect(right_x, y, button_w, button_h), "-") for y in btn_y_positions]

        # button flash
        self.button_flash = [None] * len(self.answer_buttons)
        self.button_flash_timer = [0.0] * len(self.answer_buttons)

        # cached UI interaction rects
        self.menu_buttons = {}
        self.tutorial_back = None

    def run(self):
        running = True
        while running:
            dt = self.clock.tick(FPS) / 1000.0
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    # save and exit
                    self.gs.update_save_values()
                    save_game(self.gs.save_data)
                    running = False

                elif event.type == pygame.KEYDOWN:
                    if self.state == "game":
                        if event.key == pygame.K_s:
                            self.gs.shop_open = not self.gs.shop_open
                        if event.key == pygame.K_ESCAPE:
                            # save and exit from game to menu
                            self.gs.update_save_values()
                            save_game(self.gs.save_data)
                            self.state = "menu"
                        if event.key == pygame.K_k:
                            if self.gs.items.get("skip", 0) > 0 and self.gs.tiles:
                                self.gs.items["skip"] -= 1
                                self.gs.tiles.pop(0)
                    elif self.state == "menu":
                        if event.key == pygame.K_ESCAPE:
                            self.gs.update_save_values()
                            save_game(self.gs.save_data)
                            running = False
                    else:
                        # tutorial or others: ESC back to menu
                        if event.key == pygame.K_ESCAPE:
                            self.gs.update_save_values()
                            save_game(self.gs.save_data)
                            self.state = "menu"

                elif event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    mx, my = event.pos

                    if self.state == "menu":
                        for name, rect in self.menu_buttons.items():
                            if rect.collidepoint(mx, my):
                                if name == "Play":
                                    self.state = "game"
                                    self.gs.reset_run()
                                    self.gs.spawn_tile()
                                elif name == "Tutorial":
                                    self.state = "tutorial"
                                elif name == "Quit":
                                    self.gs.update_save_values()
                                    save_game(self.gs.save_data)
                                    running = False

                    elif self.state == "tutorial":
                        if self.tutorial_back and self.tutorial_back.collidepoint(mx, my):
                            self.state = "menu"

                    elif self.state == "game":
                        if self.gs.game_over:
                            # click anywhere to restart
                            self.gs.reset_run()
                            self.gs.spawn_tile()
                            continue

                        if self.gs.shop_open:
                            # shop click areas (position preserved)
                            w, h = 700, 320
                            rect = pygame.Rect((SCREEN_W - w) // 2, (SCREEN_H - h) // 2, w, h)
                            skip_area = pygame.Rect(rect.x + 20, 48, 380, 34)
                            shield_area = pygame.Rect(rect.x + 20, 94, 380, 34)
                            if skip_area.collidepoint(mx, my):
                                if self.gs.coins >= COST_SKIP:
                                    self.gs.coins -= COST_SKIP
                                    self.gs.items["skip"] = self.gs.items.get("skip", 0) + 1
                            elif shield_area.collidepoint(mx, my):
                                if self.gs.coins >= COST_SHIELD:
                                    self.gs.coins -= COST_SHIELD
                                    self.gs.items["shield"] = self.gs.items.get("shield", 0) + 1
                            continue

                        # check answer buttons
                        for i, (rect, val) in enumerate(self.answer_buttons):
                            if rect.collidepoint(mx, my):
                                active = self._get_active_tile()
                                if not active:
                                    break
                                if val == active.answer:
                                    # flash green
                                    self._flash_button(i, GREEN)
                                    self.gs.score += 1
                                    self.gs.coins += 1
                                    if self.gs.tiles:
                                        self.gs.tiles.pop(0)
                                    self.gs.spawn_interval = max(SPAWN_INTERVAL_MIN,
                                                                SPAWN_INTERVAL_BASE - self.gs.score * SPAWN_ACCEL_PER_SCORE)
                                else:
                                    # flash red
                                    self._flash_button(i, RED)
                                    if self.gs.items.get("shield", 0) > 0:
                                        self.gs.items["shield"] -= 1
                                    else:
                                        self.gs.lives -= 1
                                        if self.gs.lives <= 0:
                                            self.gs.game_over = True
                                            if self.gs.score > self.gs.high_score:
                                                self.gs.high_score = self.gs.score
                                            self.gs.update_save_values()
                                            save_game(self.gs.save_data)
                                break


                        # skip/shield quick buttons (top-right)
                        skip_btn = pygame.Rect(SCREEN_W - 140, 10, 120, 34)
                        shield_btn = pygame.Rect(SCREEN_W - 280, 10, 120, 34)
                        if skip_btn.collidepoint(mx, my):
                            if self.gs.items.get("skip", 0) > 0 and self.gs.tiles:
                                self.gs.items["skip"] -= 1
                                self.gs.tiles.pop(0)

                # update button flash timers
                for i in range(len(self.answer_buttons)):
                    if self.button_flash[i] is not None:
                        self.button_flash_timer[i] -= dt
                        if self.button_flash_timer[i] <= 0:
                            self.button_flash[i] = None

            # --- Game update ---
            if self.state == "game" and not self.gs.game_over and not self.gs.shop_open:
                self.gs.spawn_timer += dt
                if self.gs.spawn_timer >= self.gs.spawn_interval:
                    self.gs.spawn_timer = 0
                    self.gs.spawn_tile()

                for t in self.gs.tiles:
                    t.fall_speed = BASE_FALL_SPEED + self.gs.score * SPEED_PER_SCORE
                    t.update(dt)

            # Missed tile (preserved threshold from mypygame_optimized.py)
            if self.state == "game" and self.gs.tiles and self.gs.tiles[0].y > SCREEN_H - 130:
                self.gs.tiles.pop(0)
                if self.gs.items.get("shield", 0) > 0:
                    self.gs.items["shield"] -= 1
                else:
                    self.gs.lives -= 1
                    if self.gs.lives <= 0:
                        self.gs.game_over = True
                        if self.gs.score > self.gs.high_score:
                            self.gs.high_score = self.gs.score
                        self.gs.update_save_values()
                        save_game(self.gs.save_data)

            # Update right panel choices (safe indexing)
            active = self._get_active_tile()
            if active:
                for i, (rect, _) in enumerate(self.answer_buttons):
                    val = active.choices[i] if i < len(active.choices) else "-"
                    self.answer_buttons[i] = (rect, val)
            else:
                for i, (rect, _) in enumerate(self.answer_buttons):
                    self.answer_buttons[i] = (rect, "-")

            # --- Draw ---
            if self.state == "menu":
                self.menu_buttons = self.ui.draw_menu()

            elif self.state == "tutorial":
                self.tutorial_back = self.ui.draw_tutorial()

            elif self.state == "game":
                self._draw_game()

            pygame.display.flip()

        pygame.quit()

    def _flash_button(self, index, color):
        """Temporarily flash an answer button with a given color."""
        if 0 <= index < len(self.answer_buttons):
            self.button_flash[index] = color
            self.button_flash_timer[index] = 0.025  # seconds to stay colored

    def _get_active_tile(self):
        return self.gs.tiles[0] if self.gs.tiles else None

    def _draw_game(self):
        screen = self.screen
        screen.fill((12, 12, 20))
        pygame.draw.rect(screen, (63, 63, 80), (20, 80, SCREEN_W - 260, SCREEN_H - 140), border_radius=10)

        # draw tiles
        for t in self.gs.tiles:
            t.draw(screen, self.ui.font_mid)

        # draw right panel and button flash
        self.ui.button_flash = self.button_flash
        self.ui.draw_right_panel(self.answer_buttons)

        # HUD
        self.ui.draw_hud(self.gs.score, self.gs.coins, self.gs.lives, self.gs.high_score)

        # quick item buttons
        skip_btn = pygame.Rect(SCREEN_W - 140, 10, 120, 34)
        pygame.draw.rect(screen, (70,70,70), skip_btn, border_radius=8)
        skip_label = self.ui.font_small.render(f"Skip x{self.gs.items.get('skip',0)}", True, WHITE)
        screen.blit(skip_label, (skip_btn.x + 20, skip_btn.y + 0))

        shield_btn = pygame.Rect(SCREEN_W - 280, 10, 120, 34)
        pygame.draw.rect(screen, (70,70,70), shield_btn, border_radius=8)
        shield_label = self.ui.font_small.render(f"Shield x{self.gs.items.get('shield',0)}", True, WHITE)
        screen.blit(shield_label, (shield_btn.x + 10, shield_btn.y + 0))

        shop_hint = self.ui.font_small.render("Press S to open Shop", True, GRAY)
        screen.blit(shop_hint, (SCREEN_W - 220, 52))

        if self.gs.shop_open:
            self.ui.draw_shop(self.gs.items, self.gs.coins)

        if self.gs.game_over:
            go_rect = pygame.Rect(80, 120, SCREEN_W - 160, SCREEN_H - 240)
            pygame.draw.rect(screen, (20,20,20), go_rect, border_radius=12)
            pygame.draw.rect(screen, (150,150,150), go_rect, 2, border_radius=12)
            title = self.ui.font_big.render("GAME OVER", True, RED)
            screen.blit(title, (go_rect.x + (go_rect.width - title.get_width()) // 2, go_rect.y + 30))
            sc = self.ui.font_mid.render(f"Score: {self.gs.score}", True, WHITE)
            screen.blit(sc, (go_rect.x + (go_rect.width - sc.get_width()) // 2, go_rect.y + 120))
            hi = self.ui.font_mid.render(f"High Score: {self.gs.high_score}", True, GRAY)
            screen.blit(hi, (go_rect.x + (go_rect.width - hi.get_width()) // 2, go_rect.y + 160))
            inst = self.ui.font_small.render("Click anywhere to restart", True, GRAY)
            screen.blit(inst, (go_rect.x + (go_rect.width - inst.get_width()) // 2, go_rect.y + 220))

# ----------------- Entry point ----------------- #
if __name__ == "__main__":
    GameApp().run()
